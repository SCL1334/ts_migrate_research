...
const another_js_module_1 = require("./another-js-module");
console.log('\n--- 混合依賴與失去型別保護範例 ---');
// --- 使用 js-module.js 的功能 (無 JSDoc) ---
console.log('\n[來自 js-module.js (無 JSDoc)]');
console.log('  multiply(7, 8) =', (0, js_module_1.multiply)(7, 8));
// **失去型別保護的範例**
// 因為 `multiply` 現在是 `any` 型別，TypeScript 不會對參數型別進行檢查。
// 這在編譯時不會報錯，但在執行時可能會導致非預期的行為或錯誤。
console.log('  multiply("7", "8") =', (0, js_module_1.multiply)("7", "8")); // 執行結果會是 NaN
const user = { firstName: 'Legacy', lastName: 'User' };
console.log('  getFullName(user) =', (0, js_module_1.getFullName)(user));
// 同樣地，`getFullName` 的參數 `user` 也是 `any`。
// 傳入一個不完整的物件在編譯時也不會報錯，但在執行時會產生 "undefined"。
const incompleteUser = { firstName: 'Anonymous' };
console.log('  getFullName(incompleteUser) =', (0, js_module_1.getFullName)(incompleteUser)); // 執行結果會是 "Anonymous undefined"
// --- 使用 ts-module.ts 的功能 (有型別保護) ---
console.log('\n[來自 ts-module.ts]');
console.log('  toUpperCase("hello ts") =', (0, ts_module_1.toUpperCase)('hello ts'));
// toUpperCase("hello", "ts"); // 取消註解此行會導致編譯錯誤，因為型別是明確的。
const tsProfile = { username: 'ts-user', email: 'ts@example.com' };
console.log('  displayUserProfile(tsProfile) =', (0, ts_module_1.displayUserProfile)(tsProfile));
// --- 使用 another-js-module.js 的功能 ---
console.log('\n[來自 another-js-module.js]');
console.log('  createWelcomeMessage("migrator") =', (0, another_js_module_1.createWelcomeMessage)('migrator'));
console.log('  createUserSummary() =', (0, another_js_module_1.createUserSummary)());
console.log('\n--- 範例結束 ---');
